...
api.interceptors.response.use(
    ...
    async (error) => {
        ...
        // If the failed request is the login endpoint, do not attempt token refresh or redirect.
        ...
        if (error.response && error.response.status === 401 && !originalRequest._retry && !isLoginRequest) {
            ...
            try {
                ...
                // For login failures we simply reject the promise so the component can show the error.
